...
public class ClientProxy extends ServerProxy {
    ...
    public void proxyInit() {
        ...
        for (int i = 0; i < FgtXRay.keyBind_descriptions.length; ++i) {
            FgtXRay.keyBind_keys[i] = new KeyBinding(
                FgtXRay.keyBind_descriptions[i],
                FgtXRay.keyBind_keyValues[i],
                "Fgt X-Ray");
        ...
        FMLCommonHandler.instance()
            .bus()
            .register(new ClientTick());
        MinecraftForge.EVENT_BUS.register(new RenderTick()); // RenderTick is forge subscribed to onRenderEvent. Which
                                                             // is called when drawing the world.
    }
