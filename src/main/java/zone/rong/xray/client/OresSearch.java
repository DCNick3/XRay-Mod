...
public class OresSearch {
    ...
    private static boolean checkList(List<OreInfo> temp, OreInfo value, ItemStack stack) // Used to check if a OreInfo
                                                                                         // already exists in the
                                                                                         // searchList
    {
        for (OreInfo oreCheck : temp) {
            if ((oreCheck.oreName == value.oreName) && (oreCheck.id == Item.getIdFromItem(stack.getItem()))
                && (oreCheck.meta == stack.getItemDamage())) {
                return true; // This ore already exists in the temp list. (Sometimes the OreDict returns duplicate
                             // entries, like gold twice)
            }
        }
        return false;
    }

    public static void add(String oreIdent, String name, int color) // Takes a string of id:meta or oreName to add to
                                                                    // our search list.
    {
        ...
        if (oreIdent.contains(":")) // Hopefully a proper id:meta string.
        {
            ...
            if (splitArray.length != 2) {
                ...
                String notify = String.format(
                    "[�aFgt XRay�r] %s is not a valid identifier. Try id:meta (example 1:0 for stone) or oreName (example oreDiamond or mossyStone)",
                    oreIdent);
            ...
            try {
                ...
                String notify = String.format(
                    "[�aFgt XRay�r] %s contains data other than numbers and the colon. Failed to add.",
                    oreIdent);
                ChatComponentText chat = new ChatComponentText(notify);
                mc.ingameGUI.getChatGUI()
                    .printChatMessage(chat);
    ...
    public static List<OreInfo> get() // Return the searchList, create it if needed.
    {
        if (OresSearch.searchList.isEmpty()) {
            ...
            Map<String, OreInfo> tempOredict = new HashMap<String, OreInfo>(); // Temporary oredict map to replace
                                                                               // oredictOres

            // OreDictionary.getOres("string") adds the ore if it doesn't exist.
            // Here we check our oredictOres with the untouched oredict and delete any that dont exist already. This
            // avoids polluting the oredict.
            ...
            for (Map.Entry<String, OreInfo> entry : FgtXRay.oredictOres.entrySet()) {
                ...
                ArrayList<ItemStack> oreDictOres = OreDictionary.getOres(key); // Get an itemstack array of all the
                                                                               // oredict ores for 'key'
                if (oreDictOres.size() < 1) {
                    System.out.printf(
                        "[Fgt XRay] Ore %s doesn't exist! Skipping. (We shouldn't have this issue here! Please tell me about this!)%n",
                        key);
                    continue;
                }
                for (int i = 0; i < oreDictOres.size(); i++) {
                    ...
                    temp.add(
                        new OreInfo(
                            ...
                            value.draw));
                    System.out.printf(
                        "[Fgt XRay] Adding OreInfo( %s, %d, %d, 0x%x, %b ) %n",
                        value.oreName,
                        Item.getIdFromItem(oreItem.getItem()),
                        oreItem.getItemDamage(),
                        value.color,
