...
public class DefaultConfig {
    ...
    // Default block to add. Mostly just so people can add custom blocks manually through the config until I setup a gui
    // for it.
    ...
    public static void create(Configuration config) // Put default blocks and settings into the config file.
    {
        ...
        for (Entry<String, OreInfo> ore : defaults.entrySet()) {
            ...
            String category = value.oreName.replaceAll("\\s+", "")
                .toLowerCase(); // No whitespace or capitals in the config file categories

            config.get("oredict." + category, "dictname", "SOMETHINGBROKE")
                .set(key); // We need the capitals for the ore dictionary.
            config.get("oredict." + category, "guiname", "SOMETHINGBROKE")
                .set(value.oreName);
            config.get("oredict." + category, "id", -1)
                .set(value.id);
            config.get("oredict." + category, "meta", -1)
                .set(value.meta);
            config.get("oredict." + category, "color", -1)
                .set(value.color);
            config.get("oredict." + category, "enabled", false)
                .set(value.draw);
        }

        for (OreInfo ore : custom) // Put custom block into the config file.
        {
            String name = ore.oreName.replaceAll("\\s+", "")
                .toLowerCase(); // No whitespace or capitals in the config file categories.
            config.get("customores." + name, "name", "SOMETHINGBROKE")
                .set(ore.oreName);
            config.get("customores." + name, "id", -1)
                .set(ore.id);
            config.get("customores." + name, "meta", -1)
                .set(ore.meta);
            config.get("customores." + name, "color", -1)
                .set(ore.color);
            config.get("customores." + name, "enabled", false)
                .set(ore.draw);
        }
