...
public class ConfigHandler {
    ...
    public static void setup(FMLPreInitializationEvent event) {
        ...
        config.load();
        FgtXRay.distIndex = config.get(Configuration.CATEGORY_GENERAL, "searchdist", 0)
            .getInt(); // Get our search distance.

        for (String category : config.getCategoryNames()) // Iterate through each category in our config file.
        {
            ConfigCategory cat = config.getCategory(category);

            if (category.startsWith("oredict.")) // Dont iterate over the base category and make sure were on the
                                                 // oredict category.
            {
                String dictName = cat.get("dictname")
                    .getString();
                String guiName = cat.get("guiname")
                    .getString();
                int id = cat.get("id")
                    .getInt();
                int meta = cat.get("meta")
                    .getInt();
                int color = cat.get("color")
                    .getInt();
                boolean enabled = cat.get("enabled")
                    .getBoolean(false);
    ...
    public static void add(String oreName, String ore, int color) {
        config.load();
        String formattedname = oreName.replace("\\s+", "")
            .toLowerCase();

        // check if entry exists
        for (String category : config.getCategoryNames()) {
            if (category.startsWith("customores.")) {
                if (config.get("customores." + formattedname, "name", "")
                    .getString() == formattedname) {
                    String notify = String
                        .format("[Fgt XRay] %s already exists. Please enter a different name. ", oreName);
        ...
        for (String category : config.getCategoryNames()) {
            if (category.startsWith("customores.")) {
                config.get("customores." + formattedname, "color", "")
                    .set(color);
                config.get("customores." + formattedname, "enabled", "false")
                    .set(true);
                config.get("customores." + formattedname, "id", "")
                    .set(oreId);
                config.get("customores." + formattedname, "meta", "")
                    .set(oreMeta);
                config.get("customores." + formattedname, "name", "")
                    .set(oreName);
    ...
    public static void update(String string, boolean draw) {
        ...
        {
            config.get(Configuration.CATEGORY_GENERAL, "searchdist", 0)
                .set(FgtXRay.distIndex);
            config.save();
            return;
        }
        ...
        {
            String cleanStr = string.replaceAll("\\s+", "")
                .toLowerCase(); // No whitespace or capitals in the config file categories.
            String[] splitCat = category.split("\\.");

            if (splitCat.length == 2) {
                if (splitCat[0].equals("oredict") && splitCat[1].equals(cleanStr)) // Check if the current iteration is
                                                                                   // the correct category
                                                                                   // (oredict.emerald)
                {
                    config.get("oredict." + cleanStr, "enabled", false)
                        .set(draw);

                } else if (splitCat[0].equals("customores") && splitCat[1].equals(cleanStr)) {
                    config.get("customores." + cleanStr, "enabled", false)
                        .set(draw);
                }
            }
        }
